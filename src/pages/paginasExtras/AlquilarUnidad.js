...
const AlquilarUnidad = () => {
    ...
    const handleSubmit = (e) => {
        ...
        fetch('http://localhost:8080/TPOAPI/Controller/PutAlquilarUnidad'+"/"+values.codigoEdificio+"/"+values.piso+"/"+values.numeroUnidad+"/"+values.documento, {
        method: 'PUT',
        body: formData,
        mode: "cors"
    })
    ...
    return( 
        <div className="app"> 
            <form className='form-RegistrarPersona' onSubmit={handleSubmit}>
                <h1 className='h1-RegistrarPersona' >Alquilar Unidad</h1>
                {inputs.map((input)=> (
