...
const CrearUnidad = () => {
    ...
    const handleSubmit = (e) => {
        ...
        formData.append("numero",values.numeroUnidad)
        formData.append("codigo",values.codigoEdificio)
        ...
        fetch('http://localhost:8080/TPOAPI/Controller/PostCrearUnidad', {
        method: 'POST',
        body: formData,
        mode: "cors"
    })
    ...
    return( 
        <div className="app"> 
            <form className='form-RegistrarPersona' onSubmit={handleSubmit}>
                <h1 className='h1-RegistrarPersona' >Crear Unidad</h1>
                {inputs.map((input)=> (
